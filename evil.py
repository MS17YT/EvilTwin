#!/usr/bin/env python3
"""
EVIL TWIN FRAMEWORK 3.0 - FINAL MASTER EDITION
Professional Wireless Penetration Testing Tool
- Automatic network scanning & target selection
- Fake AP creation (airbase-ng) with same SSID as target
...
- Live credential display with color
- Deauthentication attack (optional)
- Interactive command menu
- Full system cleanup (iptables, processes, monitor mode, NetworkManager)
...
INTERCEPT_HTTPS = False        # HTTPS richiede sslstrip (non incluso)
...
def check_root():
    ...
    if os.geteuid() != 0:
        ...
        print("    sudo python3 evil_twin_final.py\n")
        sys.exit(1)
...
def check_dependencies():
    ...
    if missing:
        ...
        print("[*] Installa con:")
        print(f"    sudo apt update && sudo apt install -y aircrack-ng dnsmasq tshark")
        sys.exit(1)
...
def select_target(networks):
    ...
    for idx, net in enumerate(networks, 1):
        power = net['power']
        signal_bars = "▂▄▆█"[:min(4, max(1, int((int(power)+100)//25)))] if power.isdigit() else "?"
